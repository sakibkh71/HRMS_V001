...
trait CommonService
{
    ...
    public function getAccessCompanies($company_ids){

        Artisan::call('db:connect');

        if(count($company_ids) > 0){
            $companies = Config::whereIn('id', $company_ids)->get();
            return $companies;
        }
        else{
            return "";
    ...
    public function getEmployeeByDepartmentUnitBranch($branch_id, $department_id, $unit_id, $blood_id = null, $gender_id = null, $religion_id = null, $division_id = null, $district_id = null)
    {
        $result = User::with('designation', 'designation.department')->select('users.*', DB::raw('CONCAT(users.first_name," ",users.last_name) as fullname'));
        ...
        if($blood_id > 0 || $gender_id == 'male' || $gender_id == 'female' || $religion_id > 0){

            $result->join('employee_details','employee_details.user_id','=','users.id');
        }

        if($blood_id > 0){
            $result->where('employee_details.blood_group_id','=', $blood_id);
        }

        if($gender_id == 'male' || $gender_id == 'female'){
            $result->where('employee_details.gender','=', $gender_id);
        }

        if($religion_id > 0){
            $result->where('employee_details.religion_id','=', $religion_id);
        }

        if($division_id > 0 || $district_id > 0){
            $result->join('employee_address','employee_address.user_id','=','users.id');
        }

        if($division_id > 0){
            $result->where('employee_address.permanent_division_id','=', $division_id);
        }

        if($district_id > 0){
            $result->where('employee_address.permanent_district_id','=', $district_id);
        }

        // return $result->where('users.status',1)->toSql();
        return $result->where('users.status',1)->get();
    }

    public function getAllEmployeeByDepartmentUnitBranch($branch_id, $department_id, $unit_id)
    {
        $result = User::with('designation', 'designation.department')->select('users.*', DB::raw('CONCAT(users.first_name," ",users.last_name) as fullname'));
        ...
        return $result->where('users.status','<',20)->get();
    }
