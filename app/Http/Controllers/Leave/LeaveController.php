...
class LeaveController extends Controller
{
    ...
    public function index(){
        ...
        $data['onlyBranch'] = '';
        // $data['leaves'] = EmployeeLeave::with('userName','leaveType', 'userName.designation', 'responsibleUser', 'approvedByUser')->orderBy('employee_leave_status')->get();
        $datass = DB::table('employee_leaves')->select('users.id as userId', 'users.branch_id','users.first_name as firstName', 'users.last_name as lastName', 'users.employee_no as empNo', 'designations.designation_name as designationName', 'employee_leaves.*', 'leave_types.leave_type_name as leaveTypeName')
            ->join('users', 'users.id', '=','employee_leaves.id')
            ...
            if(in_array("employee/onlyBranch", session('userMenuShare'))){
               $datass->where('users.branch_id', Auth::user()->branch_id); 
               $data['onlyBranch'] = Auth::user()->branch_id;
            }
            $datass->orderBy('employee_leaves.employee_leave_status');
            
        $data['leaves'] = $datass->get();
    ...
    public function getWeekendHolidays($fromDate, $toDate, $userId){
        ...
        if(count($weekendFromShift) > 0){
            ...
            // dd($conflictDays, $holidayWithNormalWeekend, $uniqDays);

            $data['holidays'] = $holidayCounter;
            $data['weekend'] = $weekendCounter;
            $data['weekendDays'] = $weekendDates;
            $data['holidayDays'] = $uniqDays;
        }
        else{
    ...
    public function details($emp_no=null){
        ...
        if(!empty($emp_no)){
            if(in_array("leave/add", session('userMenuShare'))){
    ...
    public function changeStatus($id, $stat){
        ...
        try { 
            ...
            $leaveDays = $this->getWeekendHolidays($val->employee_leave_from, $val->employee_leave_to, $val->user_id);

            $attendanceInfo = AttendanceTimesheet::where('user_id', $val->user_id)->whereBetween('date',[$val->employee_leave_from, $val->employee_leave_to])->get();
            // dd($attendanceInfo, $val->user_id);
            
            if($stat == 4){

                foreach($attendanceInfo as $info){
                    if(in_array($info->date, $leaveDays['weekendDays'])){
                        $observationStat = 4; //weekend
                    }
                    elseif(in_array($info->date, $leaveDays['holidayDays'])){
                        $observationStat = 3; //Holiday
                    }
                    else{
                        $observationStat = 0; //absent
                    }

                    AttendanceTimesheet::where('user_id', $val->user_id)->where('date', $info->date)->update(['observation' => $observationStat]);
                }
            }
            elseif($stat == 3){

                AttendanceTimesheet::where('user_id', $val->user_id)->whereBetween('date', [$val->employee_leave_from, $val->employee_leave_to])->update(['observation' => 2]);
            }
