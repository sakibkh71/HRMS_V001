...
class LoanController extends Controller
{
    ...
    public function index(Request $request){
        ...
        // $va = Loan::with('loanDetails', 'user','loanType','approvedBy','createdBy','updatedBy')->orderBy('id','desc')->get();
        // dd($va);

    	if($request->ajax()){
    		return Loan::with('loanDetails', 'user','loanType','approvedBy','createdBy','updatedBy')->orderBy('id','desc')->get();
    	}
    ...
    public function create(LoanRequest $request){
        try{
            ...
            $loan_deduct_amount = $request->loan_deduct_amount;

            $divResult = intdiv($loan_amount, $loan_deduct_amount);
            $modulus = fmod($loan_amount, $loan_deduct_amount);

            $durationMonth = $divResult + ($modulus > 0?1:0);
            
            $request->offsetSet('loan_duration',$durationMonth);
            $request->offsetSet('loan_or_advance',$request->loanOrAdvance);
    ...
    public function update(LoanRequest $request)
    {
        try{
            if($request->has('loan_id')){
                ...
                $request->offsetSet('loan_status', 1);
                $loan = Loan::find($id);
